...
export class OpenApi {
    ...
    private mapToResponses(responses: any): Response[] {
        return Object.entries<any>(responses).map(([statusCode, response]) => ({
            statusCode,
            schema: this.mapToSchema(response['content']['application/json']['schema'])
        }));
    }
    ...
    private mapToSchema(data: any, title?: string): Schema {
        ...
        if (data.type === 'object') {
            schema.properties = Object.entries(data.properties).map(([propKey, prop]) => this.mapToSchema(prop, propKey))
